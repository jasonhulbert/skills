---
name: plan-run
description: Execute a saved delivery-first plan while tracking delivery separately from confidence and continuing past non-blocking verification gaps. Use when the user asks to run, continue, or finish a whole saved plan, one workstream, or one work item.
---
...
Resolve the requested plan, workstream, or work item and validate its structure.
Read the increment, constraints, approved holds, decisions, dependencies,
coordination areas, delivery states, and durable notes. Auto-detect single and
multi-workstream plans.

When the user requests the whole plan, execute ready work until the selected
increment is delivered or no work can safely progress. Dependency readiness is
based only on delivery state, never confidence state. Continue independent work
when another item is blocked.
...
shared integration, conflict resolution, and final reporting with the primary
agent.
...
increment, then record the deviation and update affected notes. Ask before a
change that expands the product outcome, violates an agreed constraint, accepts
meaningful product risk, or is large enough to require replanning.

## Build confidence without chasing it

Choose proportionate checks after seeing the implementation and environment.
Use the work item's confidence targets and material risks, not a desire to make
every possible check pass. Run exact planned commands when they remain relevant
and feasible, but treat them as evidence rather than the objective.
...
is blocked and remove it when resolved. An executing agent may identify a newly
discovered concrete safety defect as an unplanned stop, but may not promote
uncertainty or missing evidence into a hold.
...
Report delivered work first, then confidence gaps, blockers or required
decisions, plan changes, and the next useful action.
